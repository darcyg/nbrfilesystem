...
static int led_sysconfig_para(void)
{
	...
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
		printk(KERN_ERR"%s: led_used script_get_item  err. \n", __func__);
		goto script_get_item_err;
	}
	led_used = val.val;

	if (1 != led_used) {
		printk(KERN_ERR"%s: led_unused.\n",  __func__);
		goto script_get_item_err;
	}

	type = script_get_item("led_para", "led_num", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
		printk(KERN_ERR"%s: led_num script_get_item err.\n", __func__);
		goto script_get_item_err;
	}
	...
	if (led_list_head == NULL) {
		printk(KERN_ERR"%s: kzalloc led list head failed!\n", __func__);
		goto script_get_item_err;
	}
	...
	if (led_list_name == NULL) {
		printk(KERN_ERR"%s: kzalloc led list name failed!\n", __func__);
		goto script_get_item_err;
	}
